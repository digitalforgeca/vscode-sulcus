...
export class AuthManager implements vscode.UriHandler {
  ...
  public handleUri(uri: vscode.Uri): vscode.ProviderResult<void> {
    // Handle vscode://digitalforgestudios.sulcus-vscode/auth?key=...&state=...
    try {
      if (uri.path === '/auth' || uri.path === 'auth' || uri.path === '') {
        const queryParams = new URLSearchParams(uri.query);
        const key = queryParams.get('key') || queryParams.get('api_key') || queryParams.get('token');
        const state = queryParams.get('state');
        const namespace = queryParams.get('namespace') || 'default';

        if (this.pendingState && state && state !== this.pendingState) {
          vscode.window.showErrorMessage('Sulcus authentication failed: CSRF state nonce mismatch.');
          return;
        }

        if (key) {
          this.completeAuth(key, namespace);
          vscode.window.showInformationMessage('Successfully connected Sulcus account to VS Code! ✓');
        }
      }
    } catch (err: any) {
      vscode.window.showErrorMessage(`Sulcus auth error: ${err.message}`);
    }
  }
  ...
  public async signIn(): Promise<void> {
    ...
    const handshakeUrl = `${baseWebUrl}/auth/handshake?client=vscode&state=${state}&port=${port || 0}&redirect_uri=${encodeURIComponent('vscode://digitalforgestudios.sulcus-vscode/auth')}`;
